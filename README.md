...
![Дерево Піфагора](assets/pythagoras_tree.png)
...
![Відображення дерева](assets/pyramid_visualization.png)
...
![Таблиця ймовірностей](assets/Monte_Carlo_method.png)
...
Прикріплені посилання на репозиторій `goit-algo-fp` та безпосередньо самі файли репозиторію у форматі `zip`.
...
## Висновки щодо правильності розрахунків методу Monte Carlo (Завдання 7)

### Результати експерименту:

![Monte Carlo vs Theoretical Probabilities](assets/result_Monte_Carlo.png)

### Аналіз точності методу Monte Carlo:

**Параметри симуляції:**
- Кількість симуляцій: 1,000,000 кидків
- Використана бібліотека: NumPy (для оптимізації продуктивності)
- Метод генерації: `np.random.default_rng().integers(1, 7)`

**Порівняння результатів:**

| Сума | Monte Carlo | Теоретична | Різниця | Абсолютна різниця |
|------|-------------|------------|---------|-------------------|
| 2    | 2.77%       | 2.78%      | -0.01%  | 0.01%            |
| 3    | 5.56%       | 5.56%      | 0.00%   | 0.00%            |
| 4    | 8.30%       | 8.33%      | -0.03%  | 0.03%            |
| 5    | 11.08%      | 11.11%     | -0.03%  | 0.03%            |
| 6    | 13.89%      | 13.89%     | 0.00%   | 0.00%            |
| 7    | 16.65%      | 16.67%     | -0.02%  | 0.02%            |
| 8    | 13.92%      | 13.89%     | +0.03%  | 0.03%            |
| 9    | 11.13%      | 11.11%     | +0.02%  | 0.02%            |
| 10   | 8.36%       | 8.33%      | +0.03%  | 0.03%            |
| 11   | 5.56%       | 5.56%      | 0.00%   | 0.00%            |
| 12   | 2.78%       | 2.78%      | 0.00%   | 0.00%            |

**Статистичні показники:**
- **Середня абсолютна різниця: 0.016%**
- **Максимальна різниця: 0.03%**
- **Найймовірніша сума (Monte Carlo):** 7 (16.65%)
- **Найймовірніша сума (теоретична):** 7 (16.67%)

### Висновки:

1. **Висока точність симуляції:** Monte Carlo метод демонструє виключно високу точність з середньою абсолютною похибкою лише 0.016%, що підтверджує правильність реалізації алгоритму.

2. **Збіжність до теоретичних значень:** Всі розраховані ймовірності збігаються з теоретичними в межах статистичної похибки (≤0.03%), що підтверджує закон великих чисел.

3. **Правильність розподілу:** 
   - Найбільша ймовірність у суми 7 (16.65%) - відповідає теоретичному максимуму
   - Найменші ймовірності у сум 2 і 12 (по ~2.78%) - відповідає теоретичному мінімуму
   - Симетричність розподілу навколо суми 7 збережена

4. **Ефективність NumPy векторизації:** Використання NumPy дозволило обробити 1 мільйон симуляцій за секунди, демонструючи високу продуктивність.

5. **Валідація методу:** Результати підтверджують, що метод Monte Carlo є надійним інструментом для оцінки ймовірностей у випадках, коли аналітичні розрахунки складні або неможливі.

**Загальний висновок:** Реалізований алгоритм Monte Carlo показує відмінні результати з похибкою менше 0.02% в середньому, що робить його надійним методом для моделювання ймовірностей кидання кубиків та підтверджує правильність як теоретичних розрахунків, так і програмної реалізації.
